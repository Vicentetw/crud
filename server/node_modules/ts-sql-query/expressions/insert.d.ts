...
declare type ReturningMultipleLastInsertedIdType<TABLE extends ITableOrView<any>> = TABLE[typeof database] extends (NoopDB | PostgreSql | SqlServer | Oracle | Sqlite) ? AutogeneratedPrimaryKeyColumnsTypesOf<TABLE> extends never ? never : () => CustomizableExecutableInsertReturning<TABLE, AutogeneratedPrimaryKeyColumnsTypesOf<TABLE>[]> : never;
declare type ReturningLastInsertedIdType<TABLE extends ITableOrView<any>> = AutogeneratedPrimaryKeyColumnsTypesOf<TABLE> extends never ? never : () => CustomizableExecutableInsertReturning<TABLE, AutogeneratedPrimaryKeyColumnsTypesOf<TABLE>>;
...
declare type DefaultValueType<TABLE extends ITableOrView<any>> = unknown extends TABLE ? () => CustomiableExecutableInsert<TABLE> : keyof RequiredColumnsForSetOf<TABLE> extends never ? () => CustomiableExecutableInsert<TABLE> : never;
